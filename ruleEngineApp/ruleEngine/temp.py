...
def create_rule(rule_string):
    ...
    return build_ast(ast_tokens, condition_pattern)
...
def build_ast(tokens, condition_pattern):
    ...
    if isinstance(tokens, str):  # Ensure we are working with a string
...
def evaluate_rule(ast, data):
    if ast['type'] == 'operand':
        print('Evaluating operand:', ast['value'])
        ...
        if actual_value is None:
            print(f"Warning: Attribute {attribute} not found in user data")
            return False
        ...
        if operator == '>':
            ...
            return actual_value == value  # For string comparison like department == "sales"

    elif ast['type'] == 'operator':
        print(f"Evaluating operator: {ast['value']}")
    ...
    return False
        async function addUser() {
            ...
            const response = await fetch('/user/', {  // Make sure this matches your URL
                method: 'POST',
                headers: {
